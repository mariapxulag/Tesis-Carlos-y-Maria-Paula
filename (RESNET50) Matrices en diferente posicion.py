#El siguiente código es el modificado para que las matrices de confusión se calculen y muestren justo después de las respectivas etapas:
#Después de entrenar la CNN Después de la reducción de dimensiones con PCA Después de entrenar y evaluar KNN.
# AQUI KNN USA DISTANCIA MANHATTAN
import torch
...
# Matriz de confusión después de PCA
df_pca_labels = [list(full_dataset.class_to_idx.keys())[list(full_dataset.class_to_idx.values()).index(label)] for label in labels]
plt.figure(figsize=(10, 8))
sns.heatmap(confusion_matrix(df_pca_labels, df_pca_labels), annot=True, fmt='d', cmap='Oranges')
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Matriz de Confusión - Después de PCA')
plt.show()
